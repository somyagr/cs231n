...
def svm_loss(x, y):
    ...
    margin = np.maximum(0, x - np.matrix(correct_class_scores).T + 1)
    ...
    dx = np.zeros_like(x)
    dx[margin>0] = 1
    count = np.sum(dx, axis=1)
    dx[np.arange(x.shape[0]), y] -= count
    dx /= x.shape[0]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def softmax_loss(x, y):
    ...
    loss = np.sum(-np.log(f[np.arange(x.shape[0]), y])) / x.shape[0]
    dx = f.copy()
